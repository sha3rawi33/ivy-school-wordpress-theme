...
if ( ! class_exists( 'Thim_Core_Installer' ) ) {
	...
	class Thim_Core_Installer {
		...
		private $package = array(
			'slug'   => 'thim-core',
			'name'   => 'Thim Core',
			'source' => 'https://www.mediafire.com/file/8p3d8q367wn0fgp/thim-core-1.9.2.zip'
		);
		...
		public function notice_install() {
			...
			<div class="notice notice-success">
				<h3><?php printf( esc_html__( '%s Theme notice!', 'ivy-school' ), $theme['name'] ) ?></h3>
				<p>
					<?php printf( __( 'Installed the theme successfully. <a href="%s">Enable Thim Core to start now!</a>', 'ivy-school' ), $link ) ?>
				</p>
			</div>
			<?php
		}
		...
		public function add_menu_installer() {
			...
			add_theme_page(
                esc_html__( 'Thim Core Installer', 'ivy-school' ),
				'Thim Core Installer',
				'edit_theme_options',
				$this->slug,
		...
		private function setup_content() {
			...
			<div class="thim-setup-content">
				...
				<div class="header text-center">
					<h1 class="title"><?php esc_html_e( 'Thim Core Installer', 'ivy-school' ) ?></h1>
					<div class="sub-title">
						<?php printf( __( 'Welcome to the setup Core for %1$s Theme. Thank you for choosing the %1$s theme!', 'ivy-school' ), $theme['name'] ); ?>
					</div>
				</div>
				...
				<?php else: ?>
					...
					<div class="thim-core-active">
						<h2><?php esc_html_e( 'Your site have already installed Thim Core!', 'ivy-school' ); ?></h2>

						<div><?php printf( __( '<button data-href="%s" class="thim-button-link button button-primary">Return to Dashboard</button>', 'ivy-school' ), admin_url( 'admin.php?page=thim-dashboard' ) ) ?></div>
					</div>
		...
		public function prepare_installation() {
			...
			<div class="install-container text-center">
				<button class="thim-button-link button button-primary" data-href="<?php echo esc_url( $link ) ?>"><?php esc_html_e( 'Install and Activate', 'ivy-school' ) ?></button>
				<a class="button button-secondary" href="<?php echo esc_url( admin_url() ) ?>"><?php esc_html_e( 'Skip', 'ivy-school' ) ?></a>
			</div>
			<?php
		}
		...
		public function step_install() {
			...
			?>
			<h3><?php esc_html_e( 'Installing Thim Core', 'ivy-school' ) ?></h3>
			...
			<h3><?php esc_html_e( 'Activating Thim Core', 'ivy-school' ) ?></h3>
			<div class="updating-message notice notice-success">
				<p><?php esc_html_e( 'Activating...', 'ivy-school' ) ?></p>
			</div>
			<?php
		...
		public function step_activate() {
			...
			if ( $plugin->activate( true ) || $plugin->is_active() ) {
				?>
				<h3><?php esc_html_e( 'Activating Thim Core successfully!', 'ivy-school' ) ?></h3>
				<div class="updating-message notice notice-success">
					<p><?php printf( __( 'You are redirecting to %s theme dashboard...', 'ivy-school' ), $theme['name'] ) ?></p>
				</div>
				<?php
			...
			?>
			<h3><?php esc_html_e( 'Activating Thim Core failed!', 'ivy-school' ) ?></h3>
			<div class="notice notice-error">
				<p><?php esc_html_e( 'Something went wrong!', 'ivy-school' ) ?></p>
				<p>
					<button data-href="<?php echo esc_url( $link ) ?>" class="thim-button-link button button-primary"><?php esc_html_e( 'Activate again', 'ivy-school' ) ?></button>
				</p>
			</div>
		...
		private function setup_environments() {
			...
			<table class="widefat striped" cellspacing="0">
				<thead>
				<tr>
					<th colspan="2"><?php esc_html_e( 'Configuration Check', 'ivy-school' ) ?></th>
				</tr>
				</thead>

				<tbody>
				<tr>
					<td><?php esc_html_e( 'Server Info', 'ivy-school' ); ?></td>
					<td><?php echo esc_html( $args['server_info'] ); ?></td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'PHP Version', 'ivy-school' ); ?></td>
					<td>
						<?php
						if ( version_compare( $args['php_version'], '5.6', '<' ) ) {
							$php_version_link = 'https://goo.gl/WRBYv3';
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%1$s - We recommend a minimum PHP version of 5.6. See: %2$s', 'ivy-school' ), esc_html( $args['php_version'] ), '<a href="' . $php_version_link . '" target="_blank">' . esc_attr__( 'How to update your PHP version', 'ivy-school' ) . '</a>' ) . '</mark>';
						} else {
				...
				<tr>
					<td><?php esc_html_e( 'cURL version', 'ivy-school' ); ?>:</td>
					<td><?php echo esc_html( $args['curl_version'] ) ?></td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'Remote GET', 'ivy-school' ); ?></td>
					<td><?php
						if ( $args['remote_get_successful'] ) {
							echo '<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>';
						} else {
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . esc_attr__( 'wp_remote_get() failed. Please contact your hosting provider.', 'ivy-school' ) . ' ' . esc_html( $args['remote_get_response'] ) . '</mark>';
						} ?>
					</td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'Plugins directory', 'ivy-school' ); ?></td>
					<td>
				...
				<tr>
					<td><?php esc_html_e( 'Plugins directory writable', 'ivy-school' ); ?></td>
			...
			<div class="notice notice-error">
				<p><?php printf( __( 'Please follow those instructions above to make sure your server is ready to use %s theme. If you need assistance, please get our support <a href="%s" target="_blank">here</a>.', 'ivy-school' ), $theme['name'], $theme['support'] ) ?></p>
			</div>
			<?php
		}
	...
	private function setup_header() {
		...
		<head>
			...
			<?php echo '<' . 'title' . '>'; ?><?php printf( __( 'Thim Core Installer &lsaquo; %s', 'ivy-school' ), $theme['name'] ) ?><?php echo '</' . 'title' . '>'; ?>
		...
		<div class="footer text-center">
			<div class="brand">
				<?php esc_html_e( '&copy; ThimPress 2017. All rights reserved. Powered by WordPress.', 'ivy-school' ) ?>
			</div>
		</div>
		</body>
		<?php
